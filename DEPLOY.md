...
> **`SESSION_SECRET` 은 반드시 넣어야 한다.** 비우면 서버 인스턴스마다 임시 키를
> 따로 만들기 때문에, 로그인한 뒤 다음 요청이 다른 인스턴스로 가면 `unauthorized` 가 뜬다.
> 장소 등록·수정이 되다 안 되다 하면 대부분 이것 때문이다.

> 환경변수는 **저장만 해서는 반영되지 않는다.** Deployments → 최신 → ⋯ → Redeploy 를 해야 한다.
...
**동시 편집** — 두 사람이 동시에 장소를 올려도 서로 덮어쓰지 않는다.
장소·유저·휴지통은 Redis HASH 에 레코드 단위로 저장하고 바뀐 것만 갱신한다.
좋아요도 SET 원자 연산이라 안전하다. 같은 장소를 동시에 수정하면 나중 저장이 이긴다.

> 예전 배열 형식으로 저장된 데이터가 있으면 첫 요청에 자동으로 옮겨진다. 할 일은 없다.
